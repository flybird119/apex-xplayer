...
extern "C" void SDL_Android_Init(JNIEnv* env, jclass cls)
{
    ...
    mEnv = env;
    //mActivityClass = cls;
	    mActivityClass = reinterpret_cast<jclass>(env->NewGlobalRef(cls));
    ...
    __android_log_print(ANDROID_LOG_INFO, "SDL", "SDL_Android_Init done!");
}
...
{
	__android_log_print(ANDROID_LOG_INFO, "SDL", "Java_org_libsdl_app_SDLActivity_onNativeResize()");
...
extern "C" int Android_JNI_OpenAudioDevice(int sampleRate, int is16Bit, int channelCount, int desiredBufferFrames)
{
	...
	JavaVM *jvm ;
	jint ret = mEnv->GetJavaVM(&jvm);
    if (ret != JNI_OK)
    {
       __android_log_print(ANDROID_LOG_INFO, "SDL", "Failed to get Java VM from JNIEnv");
        //assert(false && "Failed to get Java VM from JNIEnv");
    }

	jvm->AttachCurrentThread(&mEnv,0);
	...
	__android_log_print(ANDROID_LOG_VERBOSE, "SDL", "call java:audioInit with params %x,%d,%d,%d,%d,%d",mEnv,midAudioInit, sampleRate, audioBuffer16Bit, audioBufferStereo, desiredBufferFrames);
