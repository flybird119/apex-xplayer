...
static int queue_picture(VideoState *is, AVFrame *src_frame, double pts, int64_t pos, int serial)
{
    ...
    if (!vp->bmp || vp->reallocate || !vp->allocated ||
        ...
        if (is->videoq.abort_request && SDL_PeepEvents(&event, 1, SDL_GETEVENT, SDL_USEREVENT,SDL_USEREVENT) != 1){
            while (!vp->allocated) {
                SDL_CondWait(is->pictq_cond, is->pictq_mutex);
            }
        }
...
{
	LOGD("start ffplay main");
	argv[0] = "/sdcard/movie/small.flv";
    ...
    LOGD("parse_options done");
    input_filename="/sdcard/movie/small.flv";
    //audio_disable = 1;
    ...
    LOGD("stream opened,enter event loop");
    event_loop(is);
